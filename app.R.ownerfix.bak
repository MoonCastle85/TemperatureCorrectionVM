...
my_server <- function(input, output, session) {
  ...
  observeEvent(input$run_model, {
    ...
    tryCatch({
      ...
      temp_profiles <- read_temperature_upload(input$temp_files)
      model_fits <- fit_lm_and_knn(profile_data, stop_temp_c = input$stop_temp_c)

      predictions <- temp_profiles %>%
      ...
      showNotification(
        paste0("Processed ", n_distinct(temp_profiles$source_file), " temperature file(s)."),
        type = "message"
      )
    }, error = function(e) {
